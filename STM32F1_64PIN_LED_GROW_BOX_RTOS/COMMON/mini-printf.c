...
/* ovo je duplikat funkcije mini_itoa samo sto je prilagodena za unsigned int
 i ona se sada koristi za specifier %u u funkciji mini_vsnprintf*/
static unsigned int mini_uitoa(unsigned int value, unsigned int radix, unsigned int uppercase,
	 char *buffer, unsigned int zero_pad)
{
	char	*pbuffer = buffer;
	unsigned int	i, len;
	...
	while ((ch=*(fmt++))) {
		...
		if (ch!='%')
			...
			switch (ch) {
				...
				case 'u':
					len = mini_uitoa(va_arg(va, unsigned int), 10, 0, bf, zero_pad);
					_puts(bf, len);
					break;
